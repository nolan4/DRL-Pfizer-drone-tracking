...
class drlpDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        timesurface_tensor = torch.from_numpy(timesurface).unsqueeze(0).float() / 255.0
        stacked_image = torch.cat((frame_tensor, timesurface_tensor), dim=0) # this is correct shape so far

        # Load label and scale to resized image
        label = self.load_label(self.labels_paths[idx])

        # Perform augmentation on both image and label
        if self.augment:
            stacked_image, label = self.augment_image_label(stacked_image, label)
...
def visualize_batch(batch_images, batch_labels, batch_masks, batch_offsets, output_dir="batch_visualizations"):
    """
    Visualizes a batch of data by stacking RGB and jet-colored timesurfaces, saving RGB images, masks, and offsets.

    Args:
        batch_images: Tensor of shape [batch_size, 4, H, W] (RGB + Grayscale).
        batch_labels: Tensor of shape [batch_size, 2] (X, Y coordinates).
        batch_masks: Tensor of shape [batch_size, 1, grid_h, grid_w].
        batch_offsets: Tensor of shape [batch_size, 2, grid_h, grid_w].
    ...
    batch_labels = batch_labels.cpu().numpy()  # Convert to numpy
    ...
    for i in range(batch_size):
        ...
        # Extract RGB
        rgb_image = (batch_images[i, :, :, :3] * 255).astype(np.uint8)
        rgb_image = np.ascontiguousarray(rgb_image, dtype=np.uint8)
        rgb_image = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR)
        rgb_path = os.path.join(sample_dir, "rgb_image.png")
        cv2.imwrite(rgb_path, rgb_image)

        # Extract grayscale timesurface
        timesurface = (batch_images[i, :, :, 3] * 255).astype(np.uint8)
